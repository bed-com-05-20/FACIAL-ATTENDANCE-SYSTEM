...
export class FaceRecognitionController {
  ...
  async recognizeFace(
    @UploadedFile() file: Express.Multer.File,
    @Body('examSessionId') examSessionId: number,
  ) {
    const uploadDir = path.join(__dirname, '..', '..', 'uploads');
    fs.mkdirSync(uploadDir, { recursive: true });
    ...
    try {
      ...
      if (!session) {
        throw new BadRequestException('Invalid or missing exam session ID.');
      }

      for (const result of recognitionResults) {
        if (!result.registrationNumber) continue;

        const student = await this.studentRepo.findOne({ where: { registrationNumber: result.registrationNumber } });
        if (!student) continue;
